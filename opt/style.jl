...
using Makie: Makie, Figure, Theme, Cycle, RGBf, cgrad
...
function savefig(name::String, f::Figure; dpi=300, fig_dir="fig")
    mkpath(fig_dir)
    Makie.save(joinpath(fig_dir, name * ".pdf"), f; pt_per_unit=1, backend=CairoMakie)
    Makie.save(joinpath(fig_dir, name * ".png"), f; px_per_unit=dpi / inch, backend=CairoMakie)
    return nothing
end
